                     ...
                     hs_sandbox_config *sbc)
{
  ...
  if (!p) {
    hs_log(g_module, 2, "%s hs_output_plugin memory allocation failed",
           sbc->cfg_name);
    return NULL;
  }
  ...
  if (pthread_mutex_init(&p->cp_lock, NULL)) {
    free(p);
    hs_log(g_module, 3, "%s pthread_mutex_init failed", sbc->cfg_name);
    return NULL;
  }
  ...
  if (sbc->async_buffer_size > 0) {
    ...
    if (!p->async_cp) {
      ...
      hs_log(g_module, 2, "%s async buffer memory allocation failed",
             sbc->cfg_name);
      return NULL;
    }
  }
  ...
  p->hsb = lsb_heka_create_output(p, lua_file, state_file, ob.buf, hs_log,
...
static void shutdown_timer_event(hs_output_plugin *p)
{
  if (lsb_heka_is_running(p->hsb)) {
    if (lsb_heka_timer_event(p->hsb, time(NULL), true)) {
      hs_log(g_module, 3, "terminated: %s msg: %s", p->name,
    ...
    if (matched) {
      ...
      if (ret <= 0) {
        if (ret == LSB_HEKA_PM_SENT) {
          ...
          if (strlen(err) > 0) {
            hs_log(g_module, 4, "file: %s received: %d %s", p->name, ret, err);
          }
        }
...
static void* input_thread(void *arg)
{
  ...
  while (!p->stop) {
    ...
    if (msg) {
      ...
      int ret = output_message(p, msg);
      if (ret == LSB_HEKA_PM_RETRY) {
        while (!p->stop && ret == LSB_HEKA_PM_RETRY) {
          const char *err = lsb_heka_get_error(p->hsb);
          hs_log(g_module, 7, "retry message %llu err: %s", p->sequence_id,
                 err);
          sleep(1);
  ...
  if (p->stop) {
    ...
    hs_log(g_module, 6, "detaching: %s", p->name);
  ...
  while ((entry = readdir(dp))) {
    ...
    if (hs_load_sandbox_config(rpath, entry->d_name, &sbc, &cfg->opd, 'o')) {
      ...
      if (p) {
        ...
        hs_log(g_module, 3, "%s create_output_plugin failed", sbc.cfg_name);
      }
